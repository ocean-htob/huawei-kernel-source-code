...
static struct regulator *syn_power_vbus, *syn_power_vdd;
...
static int synaptics_gpio_setup(int gpio, bool configure, int dir, int state)
{
	...
	if (configure) {
		...
		tp_log_warning("%s: free = %d ",__func__, gpio);
		gpio_free(gpio);
	}
...
static int synaptics_get_gesture_doze_wakeup_threshold(struct synaptics_rmi4_data *rmi4_data)
{
	...
	else if(0 == strcasecmp(product_id, FW_YASSY_STR))
	{
		module_threshold = pdata->yassy_dwt;
		tp_log_info("%s: yassy_dwt = %d\n", __func__,module_threshold);
	}
	else
	{
...
void synaptics_dsx_hardware_reset(struct synaptics_rmi4_data *rmi4_data)
{
 ...
 static int synaptics_pinctrl_init(struct synaptics_rmi4_data *rmi4_data)
 {
	 ...
	 rmi4_data->pin_default =
		 pinctrl_lookup_state(rmi4_data->pinctrl, "int_default");
	 if (IS_ERR_OR_NULL(rmi4_data->pin_default)) {
		 dev_err(&client->dev, "Failed to look up default state\n");
		 return PTR_ERR(rmi4_data->pin_default);
	 }
 
	 rmi4_data->pin_sleep =
		 pinctrl_lookup_state(rmi4_data->pinctrl, "int_sleep");
...
/* Into reduce mode which can reduce power consumption*/
	...
	if (device_may_wakeup(&rmi4_data->input_dev->dev)) 
	{
		...
		{
			rmi4_data->sleep_gesture_flag = false;
			/* if easy wakeup process is still running, cancel it */
