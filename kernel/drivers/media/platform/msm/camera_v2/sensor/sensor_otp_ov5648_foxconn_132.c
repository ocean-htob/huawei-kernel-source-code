...
static  uint32_t rg_ratio_typical = 0x278;//the average of 4 Golden samples' RG ratio
static  uint32_t bg_ratio_typical = 0x2d2;//the average of 4 Golden samples' BG ratio
...
static  uint32_t ov5648_foxconn_132_otp_read_flag = 0 ;

//OV5648 opt has 2 bank
#define OV5648_FOXCONN_132_OTP_BANK0    0
#define OV5648_FOXCONN_132_OTP_BANK1    1

//bit0:module info, bit1:AWB
#define OV5648_FOXCONN_132_OTP_MODULE_INFO          1
#define OV5648_FOXCONN_132_OTP_AWB                  2
#define OV5648_FOXCONN_132_OTP_ALL  (OV5648_FOXCONN_132_OTP_MODULE_INFO | OV5648_FOXCONN_132_OTP_AWB)
...
/****************************************************************************
* FunctionName: ov5648_foxconn_132_cci_i2c_write;
* Description : i2c write interface;
***************************************************************************/
static int32_t ov5648_foxconn_132_cci_i2c_write(struct msm_sensor_ctrl_t *s_ctrl,uint32_t addr, uint16_t data)
{
	int32_t rc = -EFAULT;
...
/******************************************************************************
Function   :    ov5648_foxconn_132_otp_clear
Description:    Disable  OTP read and clear otp buffers, must called wehen read anothe bank.
******************************************************************************/
void ov5648_foxconn_132_otp_clear(struct msm_sensor_ctrl_t * s_ctrl)
{
    int i = 0;
    //Disable OTP read
    ov5648_foxconn_132_cci_i2c_write(s_ctrl, 0x3d81, 0x00);

    for ( i = 0; i < 16; i++)//16 regs in on bank
    {
        //Clear otp buffers
        ov5648_foxconn_132_cci_i2c_write(s_ctrl, 0x3d00 + i, 0x00);//0x3d00~0x3d0f
    }
...
/******************************************************************************
Function   :    ov5648_foxconn_132_select_bank
Description:    Select the specific bankNum. OTP will be read into IC's internel men.
******************************************************************************/
static int ov5648_foxconn_132_select_bank(struct msm_sensor_ctrl_t *s_ctrl, int bankNum)
{
    CMR_LOGD("%s, select bank num :%d\n",__func__, bankNum);
    if (OV5648_FOXCONN_132_OTP_BANK0 == bankNum)
    {
        ov5648_foxconn_132_cci_i2c_write(s_ctrl, 0x3d84, 0xc0);
        ov5648_foxconn_132_cci_i2c_write(s_ctrl, 0x3d85, 0x00);//bank0
        ov5648_foxconn_132_cci_i2c_write(s_ctrl, 0x3d86, 0x0f);
        ov5648_foxconn_132_cci_i2c_write(s_ctrl, 0x3d81, 0x01);//start read

        mdelay(3);//must delay 3ms
        return 0;
    }
    else if (OV5648_FOXCONN_132_OTP_BANK1 == bankNum)
    {
        ov5648_foxconn_132_cci_i2c_write(s_ctrl, 0x3d84, 0xc0);
        ov5648_foxconn_132_cci_i2c_write(s_ctrl, 0x3d85, 0x10);//bank1
        ov5648_foxconn_132_cci_i2c_write(s_ctrl, 0x3d86, 0x1f);
        ov5648_foxconn_132_cci_i2c_write(s_ctrl, 0x3d81, 0x01);//start read
...
/******************************************************************************
Function   :    ov5648_foxconn_132_otp_read_module_info
Description:    read module info OTP.
******************************************************************************/
static int ov5648_foxconn_132_otp_read_module_info(struct msm_sensor_ctrl_t *s_ctrl,
                                                                   struct ov5648_foxconn_132_otp_struct * otp_ptr)
{
    //module info from group2,bank1 first
    if (-1 == ov5648_foxconn_132_select_bank(s_ctrl, OV5648_FOXCONN_132_OTP_BANK1))
    {
        return -1;
    }
    //Module info:  Bank 1:0x3d02~0x3d06
    ov5648_foxconn_132_cci_i2c_read(s_ctrl, 0x3d00, &(otp_ptr->product_year));
    ov5648_foxconn_132_cci_i2c_read(s_ctrl, 0x3d01, &(otp_ptr->product_month));
    ov5648_foxconn_132_cci_i2c_read(s_ctrl, 0x3d02, &(otp_ptr->product_date));
    ov5648_foxconn_132_cci_i2c_read(s_ctrl, 0x3d03, &(otp_ptr->camera_id));
    ov5648_foxconn_132_cci_i2c_read(s_ctrl, 0x3d04, &(otp_ptr->supplier_version_id));
    ...
    ov5648_foxconn_132_otp_clear(s_ctrl);
    ...
    if (-1 == ov5648_foxconn_132_select_bank(s_ctrl, OV5648_FOXCONN_132_OTP_BANK0))
    {
        return -1;
    }
    //Module info: Bank 0:0x3d05~0x3d09
    ov5648_foxconn_132_cci_i2c_read(s_ctrl, 0x3d05, &(otp_ptr->product_year));
    ov5648_foxconn_132_cci_i2c_read(s_ctrl, 0x3d06, &(otp_ptr->product_month));
    ov5648_foxconn_132_cci_i2c_read(s_ctrl, 0x3d07, &(otp_ptr->product_date));
    ov5648_foxconn_132_cci_i2c_read(s_ctrl, 0x3d08, &(otp_ptr->camera_id));
    ov5648_foxconn_132_cci_i2c_read(s_ctrl, 0x3d09, &(otp_ptr->supplier_version_id));
...
/******************************************************************************
Function   :    ov5648_foxconn_132_otp_read_awb
Description:    read awb  info OTP.
******************************************************************************/
static int ov5648_foxconn_132_otp_read_awb(struct msm_sensor_ctrl_t *s_ctrl,
                                                   struct ov5648_foxconn_132_otp_struct * otp_ptr)
{
    uint32_t rg = 0;
    uint32_t bg = 0;
    ...
    ov5648_foxconn_132_cci_i2c_read(s_ctrl, 0x3d05, &(otp_ptr->wb_rg_h));
    ov5648_foxconn_132_cci_i2c_read(s_ctrl, 0x3d06, &(otp_ptr->wb_rg_l));

    ov5648_foxconn_132_cci_i2c_read(s_ctrl, 0x3d07, &(otp_ptr->wb_bg_h));
    ov5648_foxconn_132_cci_i2c_read(s_ctrl, 0x3d08, &(otp_ptr->wb_bg_l));

    ov5648_foxconn_132_cci_i2c_read(s_ctrl, 0x3d09, &(otp_ptr->wb_gbgr_h));
    ov5648_foxconn_132_cci_i2c_read(s_ctrl, 0x3d0a, &(otp_ptr->wb_gbgr_l));
    ...
    ov5648_foxconn_132_cci_i2c_read(s_ctrl, 0x3d0a, &(otp_ptr->wb_rg_h));
    ov5648_foxconn_132_cci_i2c_read(s_ctrl, 0x3d0b, &(otp_ptr->wb_rg_l));

    ov5648_foxconn_132_cci_i2c_read(s_ctrl, 0x3d0c, &(otp_ptr->wb_bg_h));
    ov5648_foxconn_132_cci_i2c_read(s_ctrl, 0x3d0d, &(otp_ptr->wb_bg_l));

    ov5648_foxconn_132_cci_i2c_read(s_ctrl, 0x3d0e, &(otp_ptr->wb_gbgr_h));
    ov5648_foxconn_132_cci_i2c_read(s_ctrl, 0x3d0f, &(otp_ptr->wb_gbgr_l));
...
/******************************************************************************
Function   :  ov5648_foxconn_132_update_otp
Description:  Check, read OTP
******************************************************************************/
static int ov5648_foxconn_132_update_otp(struct msm_sensor_ctrl_t *s_ctrl)
{
    struct ov5648_foxconn_132_otp_struct current_otp;
    ...
    memset(&current_otp, 0, sizeof(struct ov5648_foxconn_132_otp_struct));

    // R/G and B/G of current camera module is read out from sensor OTP
    ov5648_foxconn_132_read_otp(s_ctrl, &current_otp);

    ov5648_foxconn_132_otp_debug(current_otp);
    ...
    if (0 ==(ov5648_foxconn_132_otp_read_flag & OV5648_FOXCONN_132_OTP_AWB))
    {
...
/******************************************************************************
Function   :  ov5648_foxconn_132_update_awb_gain
Description:  Update the RGB AWB gain to sensor
******************************************************************************/
void ov5648_foxconn_132_update_awb_gain(struct msm_sensor_ctrl_t *s_ctrl,
                                                        uint32_t r_gain,
                                                        uint32_t g_gain,
                                                        uint32_t b_gain)
{
    CMR_LOGD("%s, R_gain:0x%x,G_gain:0x%x,B_gain:0x%x\n",__func__, r_gain, g_gain, b_gain);
    if (r_gain > 0x400) {
        ov5648_foxconn_132_cci_i2c_write(s_ctrl, 0x5186, r_gain >> 8);
        ov5648_foxconn_132_cci_i2c_write(s_ctrl, 0x5187, r_gain & 0x00ff);
    }

    if (g_gain > 0x400) {
        ov5648_foxconn_132_cci_i2c_write(s_ctrl, 0x5188, g_gain >> 8);
        ov5648_foxconn_132_cci_i2c_write(s_ctrl, 0x5189, g_gain & 0x00ff);
    }

    if (b_gain > 0x400) {
        ov5648_foxconn_132_cci_i2c_write(s_ctrl, 0x518a, b_gain >> 8);
        ov5648_foxconn_132_cci_i2c_write(s_ctrl, 0x518b, b_gain & 0x00ff);
    }
...
/******************************************************************************
Function   :  ov5648_foxconn_132_otp_func
Description:  read the otp info
******************************************************************************/
int ov5648_foxconn_132_otp_func(struct msm_sensor_ctrl_t * s_ctrl, int index)
{
	...
	if (0 == (ov5648_foxconn_132_otp_read_flag & OV5648_FOXCONN_132_OTP_AWB))
	{
		if(0 != ov5648_foxconn_132_update_otp(s_ctrl))
		{
    ...
    ov5648_foxconn_132_update_awb_gain(s_ctrl, gr_gain, gg_gain, gb_gain);
