...
static int qpnp_chg_load_battery_data(struct qpnp_lbc_chip *chip)
{
	...
	if (node)
	{
		...
		if(chip->use_other_charger)
		{
			...
			jeita_batt_param.normal.v_max = 4320;
...
#ifdef CONFIG_HUAWEI_KERNEL
/* record ti otg setup to nff log for debugging of ewp non-charging issue */
void record_nff_for_ti_otg_setup(void)
{
	int vusb_uv, vbat_uv, batt_temp, bat_present, cur_status, batt_level, bat_current;
	int reg_value[5] = {0};
	union power_supply_propval val = {0};
	struct qpnp_lbc_chip *chip = g_lbc_chip;
	u8 reg = 0;
	static int nff_record_time = 0;

	if(!(chip && chip->ti_charger))
	{
		pr_info("set otg mode before qpnp_lbc and ti-bq24152 init\n");
		return;
	}
	if(++nff_record_time > LOG_MAX_NUMBER)
	{
		pr_info("just record three times otg setup to nff log for this boot, so return directly\n");
		return;
	}
	...
	MSG_WRAPPER(CHARGE_ERROR_BASE | BOOST_ERR,
		...
		reg_value[0], reg_value[1], reg_value[2], reg_value[3], reg_value[4]);
}
#endif
...
{
	int vbat_uv, batt_temp, bat_present, cur_status, batt_level, usb_present, input_current, vusb_uv;
	int chg_type, bat_current, resume_en;
	u8 bat_sts, chg_sts, usb_sts, chg_ctrl, suspend_en;
	static int not_charge_count = 0;
	static int max_nff_count = 0;
	int mode = 0;
	int rc = 0;
	int reg_value[5] = {0};
	u8 reg = 0;
	...
	if(usb_present && (cur_status == POWER_SUPPLY_STATUS_DISCHARGING
		...
		if(not_charge_count++ < NOT_CHARGE_COUNT){
			...
			not_charge_count = 0;
			if(max_nff_count++ > NFF_MAX_COUNT)
			{
				goto skip_check_status;
			}
		}
